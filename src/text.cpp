...
void draw_num(float num, int line = 0, int xoffset = 0, bool account_for_offset = true, float size = 16.f, bool trunc = false, NRGBA tint = {1,1,1,1}) {
    ...
    while(*c) {
        if (*c >= '0' && *c <= '9') {
            ...
            gl_slow_tilemap_draw(&GET_TILEMAP_TEXTURE(font), fvec2{xoffset * size + increment, line * size}, fvec2{size, size}, 0, c_to_font, false, false, tint, account_for_offset);
            
        } else if (*c == '.') {
            gl_slow_tilemap_draw(&GET_TILEMAP_TEXTURE(font), fvec2{xoffset * size + increment, line * size}, fvec2{size, size}, 0, 14, false, false, tint, account_for_offset);
